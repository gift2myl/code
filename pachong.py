...
def main(num):
	...
	for house in soup.find_all('li',{'class':'clearfix'}):
		for price in house.find_all('p',{'class':'price'}):
			...
			ws.write(x+8*num,0,price.next_element.decode('utf-8'))
		for address in house.find_all('a',{'class':'t1'}):
			print address.next_element
			ws.write(x+8*num,1,address.next_element.decode('utf-8'))
		for detail in house.find_all('div',{'class':'detail'}):
			print detail.select('span')[0].string
			ws.write(x+8*num,2,detail.select('span')[0].string.decode('utf-8'))
			ws.write(x+8*num,3,detail.select('span')[1].string.decode('utf-8'))
			ws.write(x+8*num,4,detail.select('span')[2].string.decode('utf-8'))
		x+=1
